...
class SatyaGate(Gate):
    ...
    def check(
        ...
        return self._evidence(
            GateResult.PASSED, confidence, "No obvious misinformation patterns"
        )
...
class WitnessGate(Gate):
    ...
    def check(
        ...
        if not re.match(r"^[a-f0-9]{16}$", author_address):
            return self._evidence(
                GateResult.FAILED, 0.9, "Invalid author address format"
            )
...
class SubstanceGate(Gate):
    ...
    def check(
        ...
        if len(words) < 3:
            return self._evidence(
                GateResult.WARNING, 0.6, "Low substance: fewer than 3 words"
            )
...
class OriginalityGate(Gate):
    ...
    def check(
        ...
        if content_hash in recent_hashes:
            return self._evidence(
                GateResult.WARNING, 0.7, "Duplicate of recent content"
            )
...
class RelevanceGate(Gate):
    ...
    def check(
        ...
        if not parent_content:
            ...
            return self._evidence(
                GateResult.SKIPPED, 1.0, "No parent content to check relevance"
            )
        ...
        overlap = len(content_words & parent_words) / min(
            len(content_words), len(parent_words)
        )
        ...
        return self._evidence(
            GateResult.PASSED, 0.7, f"Relevant to parent: {overlap:.2%} overlap"
        )
...
class TelosAlignmentGate(Gate):
    ...
    def check(
        ...
        if telos_words & content_words:
            return self._evidence(
                GateResult.PASSED, 0.7, "Content aligns with declared telos"
            )

        return self._evidence(
            GateResult.WARNING, 0.5, "Content may not align with declared telos"
        )
...
class ConsistencyGate(Gate):
    ...
    def check(
        ...
        if not previous_positions:
            return self._evidence(
                GateResult.SKIPPED, 1.0, "No previous positions to check"
            )
...
class SybilGate(Gate):
    ...
    def check(
        ...
        if author_age_hours < 24 and author_reputation < 1:
            return self._evidence(
                GateResult.WARNING, 0.6, "New account with no reputation"
            )
...
class SvadhyayaGate(Gate):
    ...
    def check(
        ...
        for pattern in self.SELF_REFLECTION_PATTERNS:
            if re.search(pattern, content):
                return self._evidence(
                    GateResult.PASSED, 0.7, "Content shows self-reflection"
                )
...
class IsvaraGate(Gate):
    ...
    def check(
        ...
        for pattern in PURPOSE_PATTERNS:
            if re.search(pattern, content):
                return self._evidence(
                    GateResult.PASSED, 0.6, "Content shows purpose alignment"
                )
...
class GateProtocol:
    ...
    def verify(
        ...
        required_results = [
            e for e in evidence if e.gate_name in [g.name for g in self.required_gates]
        ]
