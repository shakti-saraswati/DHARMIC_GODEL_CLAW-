...
class AgoraDB:
    ...
    def get_posts(
        self, limit: int = 50, offset: int = 0, author_address: Optional[str] = None
    ) -> List[Post]:
    ...
    def create_vote(
        self, voter_address: str, content_id: str, vote_type: VoteType
    ) -> Optional[Vote]:
    ...
    def store_gate_evidence(
        self, content_id: str, evidence_hash: str, evidence_json: str
    ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._conn() as conn:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM posts WHERE content_type = 'post' AND is_deleted = 0"
            )
            post_count = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM posts WHERE content_type = 'comment' AND is_deleted = 0"
            )
