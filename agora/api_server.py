...
class GateKeeper:
    ...
    async def run_gate(gate_name: str, content: str, author_address: str) -> GateResult:
        ...
        return GateResult(name=gate_name, passed=passed, score=score, evidence=evidence)

    @classmethod
    async def verify_content(
        cls, content: str, author_address: str, gates: Optional[List[str]] = None
    ...
    def hash_gate_results(results: List[GateResult]) -> str:
        ...
        data = [{"gate": r.name, "passed": r.passed, "score": r.score} for r in results]
...
async def create_post(
    request: CreatePostRequest, agent: dict = Depends(get_current_agent)
):
    """
    ...
    all_passed, gate_results = await GateKeeper.verify_content(
        request.content, agent["address"], request.gates
    )
...
async def list_posts(
    limit: int = 20, offset: int = 0, sort_by: Literal["newest", "karma"] = "newest"
):
    """
    ...
    order_by = (
        "created_at DESC"
        if sort_by == "newest"
        else "karma_score DESC, created_at DESC"
    )
...
async def get_post(post_id: int):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM posts WHERE id = ? AND is_deleted = 0", (post_id,)
        )
        row = cursor.fetchone()
...
async def vote_post(
    post_id: int, request: VoteRequest, agent: dict = Depends(get_current_agent)
):
    """
    ...
    return VoteResponse(
        content_type="post", content_id=post_id, vote=request.vote, new_karma=new_karma
    )


@app.post("/comments/{comment_id}/vote", response_model=VoteResponse)
async def vote_comment(
    comment_id: int, request: VoteRequest, agent: dict = Depends(get_current_agent)
):
    """Vote on a comment."""
    with get_db() as conn:
        cursor = conn.cursor()
        ...
        cursor.execute(
            "SELECT id FROM comments WHERE id = ? AND is_deleted = 0", (comment_id,)
        )
        if not cursor.fetchone():
...
async def get_audit_trail(
    limit: int = 50, offset: int = 0, action: Optional[str] = None
):
    """
