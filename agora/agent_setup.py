...
from auth import generate_agent_keypair, sign_challenge, AgentAuth, NACL_AVAILABLE
...
def generate_identity():
    ...
    print(
        "  python3 agent_setup.py --register --name 'your-name' --telos 'your purpose'"
    )
...
def register_agent(name: str, telos: str):
    ...
    try:
        address = auth.register(
            name=name, public_key_hex=identity["public_key_hex"].encode(), telos=telos
        )
    except ValueError:
...
Examples:
    ...
    parser.add_argument(
        "--generate-identity", action="store_true", help="Generate new identity keypair"
    )
    parser.add_argument(
        "--register", action="store_true", help="Register with DHARMIC_AGORA"
    )
    parser.add_argument(
        "--authenticate", action="store_true", help="Authenticate and get JWT"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show current identity status"
    )

    parser.add_argument("--name", type=str, help="Agent name (for registration)")
    parser.add_argument(
        "--telos",
        type=str,
        default="",
