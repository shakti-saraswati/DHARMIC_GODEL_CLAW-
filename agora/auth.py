...
class AgentAuth:
    ...
    def create_challenge(self, address: str) -> bytes:
        ...
        cursor.execute(
            "SELECT address FROM agents WHERE address = ? AND is_banned = 0", (address,)
        )
    ...
    def verify_challenge(self, address: str, signature_hex: bytes) -> AuthResult:
        ...
        if not NACL_AVAILABLE:
            return AuthResult(
                success=False, error="Server misconfigured: PyNaCl missing"
            )
        ...
        return AuthResult(
            success=True, token=token, agent=agent, expires_at=expires_at.isoformat()
        )
    ...
    def _create_jwt(self, address: str, name: str, expires_at: datetime) -> str:
        ...
        signature = hmac.new(
            self._jwt_secret, message.encode(), hashlib.sha256
        ).digest()
    ...
    def delete_account(self, address: str, confirmed: bool = False) -> dict:
        """
        Delete an agent account and all associated data (REVERSIBILITY gate).

        This is a GDPR-compliant account deletion that:
        1. Exports all agent data first
        2. Removes agent from database
        3. Clears pending challenges
        4. Logs deletion to witness trail (required for accountability)

        Args:
            address: Agent's address to delete
            confirmed: Must be True to proceed (prevents accidental deletion)

        Returns:
            dict with export_data (for agent to keep) and status
        """
        if not confirmed:
            return {
                "status": "error",
                "error": "Deletion requires confirmed=True",
                "message": "This action is irreversible. Set confirmed=True to proceed.",
            }

        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        # 1. Export all agent data first (GDPR right to data portability)
        cursor.execute(
            """
            SELECT address, name, public_key_hex, created_at, reputation, telos, last_seen
        ...
        if not agent_row:
            conn.close()
            return {"status": "error", "error": f"Agent not found: {address}"}

        export_data = {
            "address": agent_row[0],
            "name": agent_row[1],
            "public_key_hex": agent_row[2],
            "created_at": agent_row[3],
            "reputation": agent_row[4],
            "telos": agent_row[5],
            "last_seen": agent_row[6],
        }

        # 2. Get witness log entries for this agent
        cursor.execute(
            """
            SELECT timestamp, action, data_hash FROM witness_log
            WHERE agent_address = ?
        """,
            (address,),
        )
        witness_entries = [
            {"timestamp": r[0], "action": r[1], "data_hash": r[2]}
            for r in cursor.fetchall()
        ]
        export_data["witness_history"] = witness_entries
        ...
        # 5. Log deletion (witness trail preserved for accountability)
        self._witness(
            "agent_deleted",
            address,
            {"reason": "user_requested", "data_exported": True},
        )

        return {
            "status": "success",
            "message": f"Account {address} deleted successfully",
            "export_data": export_data,
            "note": "Your exported data is included. Save it if needed.",
        }

    def export_account_data(self, address: str) -> dict:
        """
        Export all data for an agent (GDPR right to data portability).
