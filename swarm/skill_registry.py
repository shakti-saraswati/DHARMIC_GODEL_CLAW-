...
class RegistryCheck:
    ...
    mismatched_hashes: list[dict] = field(default_factory=list)
...
def verify_registry() -> RegistryCheck:
    ...
    hash_policy = policy.get("hashes", {}) or {}
    hashes_required = bool(hash_policy.get("required", False))
    hashes_ok = True
    missing_hashes: list[str] = []
    mismatched_hashes: list[dict] = []
    if hashes_required:
        for skill in skills:
            ...
            if not expected:
                hashes_ok = False
                missing_hashes.append(skill_id)
                continue
            if not path_str:
                hashes_ok = False
    ...
    valid = allowlist_ok and signature_valid and hashes_ok
    if valid:
        reason = "ok"
    elif not signature_valid:
        reason = "signature invalid"
    elif not allowlist_ok:
        reason = "allowlist violation"
    else:
